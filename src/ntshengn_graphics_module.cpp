...
void NtshEngn::GraphicsModule::init() {
	...
	jointMatrixBufferCreateInfo.size = 4096 * 2 * sizeof(Math::mat4);
	...
	for (auto& it : m_objects) {
		...
		if (!mesh.joints.empty()) {
			...
			if (m_playingAnimations.find(&it.second) != m_playingAnimations.end()) {
				...
				std::queue<uint32_t> joints;
				std::queue<uint32_t> parentJoints;
				joints.push(0);
				parentJoints.push(std::numeric_limits<uint32_t>::max());
				while (!joints.empty()) {
					uint32_t jointIndex = joints.front();
					Math::mat4 parentJointTransformMatrix = Math::mat4();
					if (parentJoints.front() != std::numeric_limits<uint32_t>::max()) {
						parentJointTransformMatrix = jointTransformMatrices[parentJoints.front()];
					}

					if (animation.jointKeyframes.find(jointIndex) != animation.jointKeyframes.end()) {
						uint32_t keyframe = 0;
						
						// Loop animation
						if (playingAnimation.time > animation.jointKeyframes.at(jointIndex).back().timestamp) {
							playingAnimation.time = std::fmod(playingAnimation.time, animation.jointKeyframes.at(jointIndex).back().timestamp);
						}

						// Find previous keyframe
						for (size_t i = 0; i < animation.jointKeyframes.at(jointIndex).size() - 1; i++) {
							if (playingAnimation.time == animation.jointKeyframes.at(jointIndex)[i + 1].timestamp) {
								keyframe = static_cast<uint32_t>(i) + 1;
								break;
							}
							else if (playingAnimation.time < animation.jointKeyframes.at(jointIndex)[i + 1].timestamp) {
								keyframe = static_cast<uint32_t>(i);
								break;
							}
						}

						if (animation.interpolationType == AnimationInterpolationType::Step) {
							// Step interpolation
							const AnimationJointTransform& jointTransform = animation.jointKeyframes.at(jointIndex)[keyframe].jointTransform;

							const Math::mat4 jointTransformMatrix = Math::translate(jointTransform.translation) *
								Math::to_mat4(jointTransform.rotation) *
								Math::scale(jointTransform.scale);

							jointTransformMatrices[jointIndex] = parentJointTransformMatrix *
								jointTransformMatrix *
								mesh.joints[jointIndex].baseTransform;
						}
						else if (animation.interpolationType == AnimationInterpolationType::Linear) {
							// Linear interpolation
							const AnimationJointTransform& jointTransformPrevious = animation.jointKeyframes.at(jointIndex)[keyframe].jointTransform;

							if ((keyframe + 1) >= animation.jointKeyframes.at(jointIndex).size()) {
								// Last keyframe
								const Math::mat4 jointTransformMatrix = Math::translate(jointTransformPrevious.translation) *
									Math::to_mat4(jointTransformPrevious.rotation) *
									Math::scale(jointTransformPrevious.scale);
								...
								const AnimationJointTransform& jointTransformNext = animation.jointKeyframes.at(jointIndex)[keyframe + 1].jointTransform;

								const float timestampPrevious = animation.jointKeyframes.at(jointIndex)[keyframe].timestamp;
								const float timestampNext = animation.jointKeyframes.at(jointIndex)[keyframe + 1].timestamp;
								const float interpolationValue = 1.0f - ((timestampNext - playingAnimation.time) / (timestampNext - timestampPrevious));

								const Math::vec3 translation = Math::vec3(Math::lerp(jointTransformPrevious.translation.x, jointTransformNext.translation.x, interpolationValue),
									Math::lerp(jointTransformPrevious.translation.y, jointTransformNext.translation.y, interpolationValue),
									Math::lerp(jointTransformPrevious.translation.z, jointTransformNext.translation.z, interpolationValue));
								const Math::quat rotation = Math::slerp(jointTransformPrevious.rotation, jointTransformNext.rotation, interpolationValue);
								const Math::vec3 scale = Math::vec3(Math::lerp(jointTransformPrevious.scale.x, jointTransformNext.scale.x, interpolationValue),
									Math::lerp(jointTransformPrevious.scale.y, jointTransformNext.scale.y, interpolationValue),
									Math::lerp(jointTransformPrevious.scale.z, jointTransformNext.scale.z, interpolationValue));
						...
						jointTransformMatrices[jointIndex] = parentJointTransformMatrix * mesh.joints[jointIndex].baseTransform;
					}

					for (uint32_t jointChild : mesh.joints[jointIndex].children) {
						joints.push(jointChild);
						parentJoints.push(jointIndex);
					}
...
NtshEngn::MeshID NtshEngn::GraphicsModule::load(const Mesh& mesh) {
	...
	if (!mesh.joints.empty()) {
		for (size_t i = 0; i < mesh.joints.size(); i++) {
			...
			jointMatrices.push_back(mesh.joints[i].inverseGlobalTransform);
		}
		...
		jointMatrixStagingBufferCreateInfo.size = mesh.joints.size() * sizeof(Math::mat4) * 2;
		...
		memcpy(data, jointMatrices.data(), mesh.joints.size() * sizeof(Math::mat4) * 2);
	...
	if (!mesh.joints.empty()) {
		...
		jointMatrixBufferCopy.dstOffset = m_currentJointOffset * sizeof(Math::mat4) * 2;
		jointMatrixBufferCopy.size = mesh.joints.size() * sizeof(Math::mat4) * 2;
...
void NtshEngn::GraphicsModule::playAnimation(Entity entity, uint32_t animationIndex) {
	if (!ecs->hasComponent<Renderable>(entity)) {
		NTSHENGN_MODULE_WARNING("Entity " + std::to_string(entity) + " does not have a Renderable component, when trying to play animation " + std::to_string(animationIndex) + ".");
		return;
	}
	...
	if (animationIndex >= renderable.model->animations.size()) {
		NTSHENGN_MODULE_WARNING("Animation " + std::to_string(animationIndex) + " does not exist for Entity " + std::to_string(entity) + "\'s model.");
		return;
	}
...
void NtshEngn::GraphicsModule::createColorAndDepthImages() {
	...
	m_vkCmdPipelineBarrier2KHR(m_initializationCommandBuffer, &undefinedToColorAttachmentOptimalAndUndefinedToDepthStencilAttachmentOptimalDependencyInfo);
	...
	const std::string vertexShaderCode = R"GLSL(
		...
		void main() {
			...
			if (meshes.info[objects.info[oID.objectID].meshID].hasSkin == 1) {
				...
				mat4 joint0Matrix = jointMatrices.info[jointOffset + joints.x].inverseGlobalTransform * jointTransforms.matrix[jointTransformOffset + joints.x] * jointMatrices.info[jointOffset + joints.x].inverseBindMatrix;
				mat4 joint1Matrix = jointMatrices.info[jointOffset + joints.y].inverseGlobalTransform * jointTransforms.matrix[jointTransformOffset + joints.y] * jointMatrices.info[jointOffset + joints.y].inverseBindMatrix;
				mat4 joint2Matrix = jointMatrices.info[jointOffset + joints.z].inverseGlobalTransform * jointTransforms.matrix[jointTransformOffset + joints.z] * jointMatrices.info[jointOffset + joints.z].inverseBindMatrix;
				mat4 joint3Matrix = jointMatrices.info[jointOffset + joints.w].inverseGlobalTransform * jointTransforms.matrix[jointTransformOffset + joints.w] * jointMatrices.info[jointOffset + joints.w].inverseBindMatrix;
...
void NtshEngn::GraphicsModule::createDescriptorSets() {
	...
	for (uint32_t i = 0; i < m_framesInFlight; i++) {
		...
		jointMatricesDescriptorBufferInfo.range = 4096 * 2 * sizeof(Math::mat4);
